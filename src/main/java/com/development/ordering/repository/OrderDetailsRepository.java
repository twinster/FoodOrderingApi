...
public interface OrderDetailsRepository extends CrudRepository<OrderDetails, Long> {
    ...
    @Query(value =  "SELECT order_details.* FROM orders\n" +
                    ...
                    "WHERE orders.valid_from <= :date AND orders.valid_to >= :date AND order_status.english_name = 'Confirmed'",
            nativeQuery = true)
    public List<OrderDetails> getOrderDetails(@Param("date") Date date);
}
